...
- [x] READMEをGitHubの利用者向け入口として再構成する。
- [x] CI、脆弱性報告、コントリビューション案内を追加する。
- [x] ルート直下の配布アーカイブをGit除外し、誤コミットを防ぐ。
- [x] 公開前ゲート（160テスト、Releaseビルド、文書リンク、履歴・追跡対象検査）を再実行する。
- [x] 公開準備の変更を確認し、コミットを作成する。
- [x] 利用者の明示承認後に`main`をGitHubへpushする。

完了条件:

- ローカルの全テスト、Releaseビルド、文書リンク検査が成功する。
- 全Git履歴に旧EXE・DLL、配布ZIP、秘密情報、個人パスが存在しない。
- `origin`が`https://github.com/moe-charm/EasyHTTPServer.git`を指す。
- push前にコミットと送信範囲を利用者へ提示する。
...
- Git remoteは公開準備完了後に設定し、明示承認なしにpushしない。
